class Solution:
    def kidsWithCandies(self, candies: list[int], extraCandies: int) -> list[bool]:
        """We get initial highest amount of candies from the list"""
        default_highest = max(candies)

        """We enumerate the list in order to change value at same loop as checking it"""
        for index, number in enumerate(candies):

            if number + extraCandies >= default_highest:  # we need to only check whether it's higher/equals of initial maximum candies
                candies[index] = True
            else:
                candies[index] = False
